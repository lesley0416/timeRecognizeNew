...
public class TimeNormalizer implements Serializable {
	 ...
	 * 返回替换时间表达式后的文本
	 * 替换格式：
	 * 预处理后的例句：08年患癌症，09年治愈。
	 * 【原计划这样，后改成下面的】替换后的内容：TIME患癌症，TIME治愈。|||\001 TIME \002 1 \002 0 \002 3 \002 08年 \002 2008年 \001 TIME \002 2 \002 7 \002 10 \002 09年 \002 2009年
	 * 
	 * @author Shi Lixin
	 * @return 返回替换后的字符串
	 */
	public String getTagString() {
		...
		for (int i = 0; i < timeToken.length; i++) {
			String str_i = "" + (i+1);
			result = result.substring(0, timeToken[i].startPos+posMiss) + "_TIME" + str_i + "_" + result.substring(timeToken[i].endPos + posMiss);
			result = result + "\001" + "TIME" + str_i + 
					"\002" + timeToken[i].Time_Expression;
			if (timeToken[i].Time_Norm == "") {
				result = result + "\002" + "null";
			}
			else {
				result = result + "\002" + timeToken[i].Time_Norm;
			}
			
			posMiss = posMiss + "_TIME_".length() + str_i.length() - timeToken[i].Time_Expression.length();
		}		
//		int posMiss = 0;
//		for (int i = 0; i < timeToken.length; i++) {
//			result = result.substring(0, timeToken[i].startPos+posMiss) + "TIME" + result.substring(timeToken[i].endPos+posMiss);
//			result = result + 
//					"\001" + "TIME" + "\002" + (i+1) + 
//					"\002" + timeToken[i].startPos + "\002" + timeToken[i].endPos + 
//					"\002" + timeToken[i].Time_Expression ;
//			if (timeToken[i].Time_Norm == "") {
//				result = result + "\002" + "null";
//			}
...
//				result = result + "\002" + timeToken[i].Time_Norm;
//			}
//			posMiss = posMiss + "TIME".length() - timeToken[i].Time_Expression.length();
//		}
...
//		target = stringPreHandlingModule.numberTranslator(target);// 大写数字转化
	...
	private TimeUnit[] TimeEx(String tar, String timebase) {
		...
		int [] tempStart = new int[99];
		int [] tempEnd = new int[99];
...
//		}
		...
		for (int j = 0; j < rpointer; j++) {
			 ...
			 */
			Time_Result[j]=new TimeUnit(temp[j], tempStart[j], tempEnd[j], this);
...
//		Time_Result = filterTimeUnit(Time_Result);
	...
	public static TimeUnit[] filterTimeUnit(TimeUnit[] timeUnit){
        ...
        for(TimeUnit t : timeUnit){
        	// 没有进行标准化过程的结果对应的time是null
        	// 进行标准化过程之后，无法正确识别的时间是1970.01.01 00:00:00(fastTime=-28800000)
        	if(t.getTime() == null || t.getTime().getTime() != -28800000){
        		list.add(t);
        	}
        }
	...
	/****
	 * 对模型文件进行更新，加正则什么的（貌似每次加都要调试程序进行验证，好像也没必要留这个口，，，）
	 * @param oriModelPath
	 * @param newModelPath
	 * @param newRegular
	 */
	public static void updateModel(String oriModelPath, String newModelPath, String newRegular) {
		return;
	}
...
//
//		normalizer.parse("哈哈，2012-07行动，2个半月前，儿时，10余年前，一个半月前，一个半月之后走，十余年十二指肠溃疡，这是十年余了，前天早上走，过去3年，本周日到下周日出差");// 抽取时间
